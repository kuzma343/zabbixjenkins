    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'docker run -d --name zabbix-web -e DB_SERVER_HOST=mariadb -e MYSQL_USER=your_user -e MYSQL_PASSWORD=your_password -e MYSQL_DATABASE=your_database_name -e ZBX_SERVER_HOST=zabbix-server -p 8080:8080 -p 443:443 kuzma343/zabbix-web:alpine-6.2-latest'
            }
        }
    }
}
